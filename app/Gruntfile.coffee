module.exports = (grunt, options) ->
  ...
  grunt.initConfig
    ...
    'concurrent': {
      dev: {
        tasks: ['nodemon:dev', 'nodemon:front', 'watch'],
        options: {
    ...
    'clean':
      ...
      'po_js': ['lang_src/*.po'],
      'dist': ['dist/*', 'dist/', 'src/modules/**/public/css/*.css'],
...
# Header
    ...
    'watch' : {
      config: {
        files: ['src/modules/**/config/*.yml.local', 'config/*.yml.local'],
        tasks: ['build:config'],
        options: {
          livereload: true
        }
      ...
      sass: {
        files: ['src/modules/**/public/css/*.scss'],
        tasks: ['build:sass'],
        options: {
          livereload: true
        }
      ...
      src: {
        files: ['src/modules/**/public/*.dot'],
        tasks: ['build:templates.js'],
        options: {
          livereload: true
        }
      },
      ...
      po: {
        files: ['lang_src/common/*.po', 'src/modules/**/lang_src/*.po'],
        tasks: ['build:i18n'],
        options: {
          livereload: true
        }
  ...
  grunt.registerTask 'build',               ['clean:dist', 'transpile', 'build:sass', 'build:config', 'build:i18n', 'build:templates.js', 'build:images', 'concurrent']
  grunt.registerTask 'build:config',        ['clean:config', 'yaml2json:config', 'preprocess:config']
  grunt.registerTask 'build:templates.js',  ['dot-generate', 'dot', 'js-generate', 'copy']
  grunt.registerTask 'build:images',        ['images-generate']
